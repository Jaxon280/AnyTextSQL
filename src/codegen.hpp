...
class Codegen {
    ...
    void add_ltable(Qlabel &ql) {
        ...
        if (ql.kind == ORDERED) {
            for (int i = 0; i < 17; i++) {
                code += "&&q";
                code += std::to_string(qlabels[ql.delta->r_table[i]].state);
                if (i <= 16 - ql.delta->str.size()) {
                    code += "_sink";
                }
            ...
            for (int i = 1; i < ASCII_SZ; i++) {
                code += "&&q";
                code += std::to_string(qlabels[ql.delta->char_table[i]].state);
                if (ql.delta->startState != ql.delta->char_table[i] &&
                    ql.delta->char_table[i] != INV_STATE) {
                    code += "_inc";
                }
